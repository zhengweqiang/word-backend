...
export function PaperTemplatePage() {
    ...
    return <section class="space-y-5">
        ...
        <div class="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(300px,0.7fr)]"><div class="overflow-x-auto rounded-md border bg-background"><Show when={!loading()} fallback={<p class="p-5 text-sm text-muted-foreground">正在加载试卷...</p>}><Show when={papers().length} fallback={<EmptyState title="暂无试卷" description="创建第一份试卷后，从共享题库添加试题。" />}><table class="w-full min-w-[820px] text-left text-sm"><thead class="bg-muted"><tr><th class="px-3 py-2">标题</th><th class="px-3 py-2">类型</th><th class="px-3 py-2">题数</th><th class="px-3 py-2">总分</th><th class="px-3 py-2">状态</th><th class="px-3 py-2 text-right">操作</th></tr></thead><tbody><For each={papers()}>{(paper) => <tr class="border-t"><td class="px-3 py-3"><p class="font-medium">{paper.title}</p><p class="text-xs text-muted-foreground">{paper.instructions || "无作答说明"}</p></td><td class="px-3 py-3 whitespace-nowrap">{categoryText(paper.categories)}</td><td class="px-3 py-3">{paper.questionCount}</td><td class="px-3 py-3">{paper.totalScore}</td><td class="px-3 py-3"><Badge variant="outline">{statusLabel[paper.status]}</Badge></td><td class="px-3 py-2"><div class="flex justify-end gap-1"><Button size="icon" variant="ghost" aria-label={`预览 ${paper.title}`} title="预览" onClick={() => setSelected(paper)}><Eye class="h-4 w-4" /></Button><Show when={canEdit(paper)}><A class="inline-flex h-9 w-9 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground" aria-label={`编辑 ${paper.title}`} title="编辑" href={`/papers/${paper.id}/edit`}><Pencil class="h-4 w-4" /></A></Show><Button size="icon" variant="ghost" aria-label={`复制 ${paper.title}`} title="复制" onClick={() => void copy(paper)}><Copy class="h-4 w-4" /></Button><Show when={canArchive(paper) && paper.status !== "ARCHIVED"}><Button size="icon" variant="ghost" aria-label={`归档 ${paper.title}`} title="归档" onClick={() => void archive(paper)}><Archive class="h-4 w-4" /></Button></Show></div></td></tr>}</For></tbody></table></Show></Show></div>
            <aside class="rounded-md border bg-background p-4"><Show when={selected()} fallback={<p class="text-sm text-muted-foreground">选择试卷查看配置与题目摘要。</p>}>{(paper) => <div class="space-y-3"><h2 class="text-lg font-semibold">{paper().title}</h2><div class="grid grid-cols-2 gap-2 text-sm"><p>类型：{categoryText(paper().categories)}</p><p>题目：{paper().questionCount}</p><p>总分：{paper().totalScore}</p><p>打乱题目：{paper().shuffleQuestions ? "是" : "否"}</p><p>打乱选项：{paper().shuffleOptions ? "是" : "否"}</p></div><ol class="space-y-2"><For each={paper().questions}>{(question) => <li class="rounded-md bg-muted p-2 text-sm">{question.questionOrder}. {question.stem} <span class="text-muted-foreground">({question.score} 分)</span></li>}</For></ol></div>}</Show></aside>
        </div>
    </section>;
}
