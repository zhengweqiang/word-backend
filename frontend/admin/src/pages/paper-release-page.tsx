...
export function PaperReleasePage() {
    ...
    return <section class="space-y-5">
        ...
        <div class="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(340px,0.7fr)]">
            <div data-testid="release-table-scroll" class="overflow-x-auto rounded-md border bg-background"><Show when={releases().length} fallback={<EmptyState title="暂无发布记录" description="发布可用试卷后，记录会保存在这里。" />}><table class="w-full min-w-[720px] text-left text-sm"><thead class="bg-muted"><tr><th class="px-3 py-2">发布</th><th class="px-3 py-2">状态</th><th class="px-3 py-2">时间</th><th class="px-3 py-2">目标</th><th class="px-3 py-2">结果</th></tr></thead><tbody><For each={releases()}>{(item) => <tr class="border-t" classList={{ "bg-accent/40": selectedReleaseId() === item.id }}><td class="px-3 py-3"><button type="button" class="text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`选择发布 ${item.title}`} onClick={() => setSelectedReleaseId(item.id)}><span class="block font-medium">{item.title}</span><span class="text-xs text-muted-foreground">#{item.id}</span></button></td><td class="px-3 py-3"><Badge variant="outline">{statusLabels[item.status]}</Badge></td><td class="px-3 py-3 text-xs"><p>{item.startTime || "立即开始"}</p><p class="text-muted-foreground">至 {item.deadline || "不限时"}</p></td><td class="px-3 py-3">{item.targets.length} 人</td><td class="px-3 py-3"><A class="text-primary underline" href={`/paper-results/${item.id}`}>查看结果</A></td></tr>}</For></tbody></table></Show></div>
            <aside class="space-y-4 rounded-md border bg-background p-4">
                <Show when={selectedRelease()} fallback={<p class="text-sm text-muted-foreground">选择发布记录查看详情和纠正操作。</p>}>{(item) => <>
                    <div><div class="flex items-center justify-between"><h2 class="font-semibold">{item().title}</h2><Badge>{statusLabels[item().status]}</Badge></div><dl class="mt-3 grid grid-cols-2 gap-2 text-sm"><div><dt class="text-muted-foreground">题目</dt><dd>{item().questionCount}</dd></div><div><dt class="text-muted-foreground">总分</dt><dd>{item().totalScore}</dd></div><div><dt class="text-muted-foreground">空白规则</dt><dd>{item().blankAnswerPolicy === "ALLOW_BLANK" ? "允许留空" : "全部必答"}</dd></div><div><dt class="text-muted-foreground">结果</dt><dd>{resultVisibilityLabels[item().resultVisibility]}</dd></div></dl></div>
