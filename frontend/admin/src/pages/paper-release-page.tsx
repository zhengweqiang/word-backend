        ...
        <div class="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(340px,0.7fr)]">
            ...
            <aside class="space-y-4 rounded-md border bg-background p-4">
                <Show when={selectedRelease()} fallback={<p class="text-sm text-muted-foreground">选择发布记录查看详情和纠正操作。</p>}>{(item) => <>
                    <div><div class="flex items-center justify-between"><h2 class="font-semibold">{item().title}</h2><Badge>{statusLabels[item().status]}</Badge></div><dl class="mt-3 grid grid-cols-2 gap-2 text-sm"><div><dt class="text-muted-foreground">题目</dt><dd>{item().questionCount}</dd></div><div><dt class="text-muted-foreground">总分</dt><dd>{item().totalScore}</dd></div><div><dt class="text-muted-foreground">空白规则</dt><dd>{item().blankAnswerPolicy === "ALLOW_BLANK" ? "允许留空" : "全部必答"}</dd></div><div><dt class="text-muted-foreground">结果</dt><dd>{item().resultVisibility}</dd></div></dl></div>
