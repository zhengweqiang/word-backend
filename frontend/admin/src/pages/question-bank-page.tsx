...
export function QuestionBankPage() {
    const auth = useAuth();
    const [filters, setFilters] = createStore({ keyword: "", questionType: "", status: "ACTIVE", tag: "" });
    ...
    const openEdit = (item: QuestionBankItemResponse) => {
        ...
        setForm({
            questionType: item.questionType, stem: item.stem,
    ...
    return (
        <section class="space-y-5">
            ...
            <div class="grid gap-3 rounded-md border bg-background p-3 lg:grid-cols-[minmax(220px,1fr)_170px_150px_180px_auto]">
                ...
                <div data-testid="question-table-scroll" class="overflow-x-auto rounded-md border bg-background">
                    <Show when={!loading()} fallback={<p class="p-5 text-sm text-muted-foreground">正在加载题库...</p>}>
                        <Show when={items().length} fallback={<EmptyState title="没有匹配试题" description="调整筛选条件或创建一条新试题。" />}>
                            <table class="w-full min-w-[760px] text-left text-sm"><thead class="bg-muted/70 text-xs text-muted-foreground"><tr><th class="px-3 py-2">题型</th><th class="px-3 py-2">题干</th><th class="px-3 py-2">分值</th><th class="px-3 py-2">状态</th><th class="px-3 py-2 text-right">操作</th></tr></thead><tbody>
                                <For each={items()}>{(item) => <tr class="border-t align-top"><td class="px-3 py-3 whitespace-nowrap">{questionTypeLabels[item.questionType]}</td><td class="max-w-xl px-3 py-3"><p class="font-medium">{item.stem}</p><p class="mt-1 text-xs text-muted-foreground">{(item.tags ?? []).join(" · ") || "无标签"}</p></td><td class="px-3 py-3">{item.defaultScore}</td><td class="px-3 py-3"><Badge variant="outline">{statusLabels[item.status]}</Badge></td><td class="px-3 py-2"><div class="flex justify-end gap-1"><Button size="icon" variant="ghost" title="预览" aria-label={`预览 ${item.stem}`} onClick={() => setSelected(item)}><Eye class="h-4 w-4" /></Button><Show when={canManage(item)}><Button size="icon" variant="ghost" title="编辑" aria-label={`编辑 ${item.stem}`} onClick={() => openEdit(item)}><Pencil class="h-4 w-4" /></Button></Show><Button size="icon" variant="ghost" title="复制" aria-label={`复制 ${item.stem}`} onClick={() => void copyItem(item)}><Copy class="h-4 w-4" /></Button><Show when={canManage(item) && item.status !== "ARCHIVED"}><Button size="icon" variant="ghost" title="归档" aria-label={`归档 ${item.stem}`} onClick={() => void archiveItem(item)}><Archive class="h-4 w-4" /></Button></Show></div></td></tr>}</For>
                            </tbody></table>
                        </Show>
                    </Show>
                </div>
                <aside class="rounded-md border bg-background p-4">
                    <Show when={selected()} fallback={<p class="text-sm text-muted-foreground">选择一条试题查看答案、选项和解析。</p>}>{(item) => <div class="space-y-4"><div><Badge>{questionTypeLabels[item().questionType]}</Badge><h2 class="mt-3 text-lg font-semibold">{item().stem}</h2></div><Show when={Object.keys(item().options ?? {}).length}><div class="space-y-2"><For each={Object.entries(item().options ?? {})}>{([key, value]) => <p class="rounded-md bg-muted px-3 py-2 text-sm">{key}：{value}</p>}</For></div></Show><p class="text-sm"><span class="text-muted-foreground">正确答案：</span>{item().acceptedAnswers.join("、")}</p><p class="text-sm"><span class="text-muted-foreground">解析：</span>{item().explanation || "暂无解析"}</p></div>}</Show>
                </aside>
            </div>
            <Show when={showForm()}>
